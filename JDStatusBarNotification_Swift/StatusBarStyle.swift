...
@objc
public class StatusBarStyle : NSObject {
    ...
    class func allDefaultBarStyles() -> [BarStylePreset] {
        return [.Error, .Warning, .Success, .Matrix, .Dark]
    }

    class func statusBarStyleForPreset(_ presetName: BarStylePreset) -> StatusBarStyle {
        ...
        switch presetName {
            ...
            case .Success:
                ...
                style.progressBarHeight = 1.0 + (1.0/UIScreen.main.scale)
            case .Dark:
                style.barColor = UIColor(red: 0.05, green: 0.078, blue: 0.12, alpha: 1.0)
                style.textColor = UIColor(white: 0.95, alpha: 1.0)
                style.progressBarHeight = 1.0 + (1.0/UIScreen.main.scale)
            case .Matrix:
                style.barColor = .black
                style.textColor = .green
                style.font = .init(name: "Courier-Bold", size: 14.0)!
                style.progressBarColor = .green
                style.progressBarHeight = 2.0
            case .Default:
                break
        }
